...
public class Scene extends JPanel implements ActionListener {
    ...
    public void paintComponent(Graphics g) {
        ...
        try {
             ...
             leadBalls.forEach((ball) -> {
                 Ellipse2D.Double e = new Ellipse2D.Double(ball.getPosition().x-Ball.SIZE/2.0, ball.getPosition().y-Ball.SIZE/2.0, Ball.SIZE, Ball.SIZE);
                 g2draw.setColor(ball.getColor());
                 g2draw.fill(e);
             });
            ...
            connectors.forEach((connector) -> {
                Line2D.Double c = new Line2D.Double(connector.getStart(), connector.getEnd());
                g2draw.setColor(connector.getColor());
                g2draw.draw(c);
            });
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        for (int i = 0; i < leadBalls.size(); i++) {
            ...
            for (int j = i+1; j < leadBalls.size(); j++) {
                if (Math.abs(Ball.distance(ball.getPosition(), leadBalls.get(j).getPosition())) < Ball.CONNECTION_VALUE) {
                    connectors.add(new Connector((Point2D.Double) ball.getPosition().clone(), (Point2D.Double) leadBalls.get(j).getPosition().clone(), this));
                }
            }
        ...
        for (int i = trailBalls.size()-1; i >= 0; i--) {
            ...
            if (trailBalls.get(i).getColor().getAlpha()<=0) {
                trailBalls.remove(i);
            }
        }
        ...
        for (int i = connectors.size()-1; i >= 0; i--) {
            ...
            if (connectors.get(i).getColor().getAlpha()<=0) {
                connectors.remove(i);
            }
        }
